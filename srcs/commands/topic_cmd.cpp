...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/20 03:36:51 by fatilly           #+#    #+#             */
/*   Updated: 2022/08/23 03:30:40 by fatilly          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
	...
	else
	{
		if (msg.getArgs().size() == 2)
		{
			if (ischannelExists(msg, msg.getArgs()[1].substr(1, msg.getArgs()[1].size() - 1)))
			{
				...
				if (msg.getServer()->getChannels()[i]->getTopic().size() > 0)
					sendMessage(msg, msg.getArgs()[0], msg.getServer()->getChannels()[i]->getTopic().c_str(), msg.getAuthor()->getId());
			}
			else
		...
		else if (msg.getArgs().size() == 3)
		{
			...
			{
				i = findChannel(msg, msg.getArgs()[1].substr(1, msg.getArgs()[1].size() - 1));
				msg.getServer()->getChannels()[i]->addTopic(msg.getArgs()[2], *msg.getAuthor());
			}
			else
				sendErrorMessage(msg, NOCHAN, msg.getArgs()[1], "No such channel");
		}
